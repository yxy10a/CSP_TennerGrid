...
def prop_FC(csp, newVar=None):
    ...
    if not newVar:
        ...
        for c in csp.get_all_cons():
            if c.get_n_unasgn() == 1:
                var_to_check = c.get_unasgn_vars()[0]
                vars = c.get_scope()
                for val in var_to_check.cur_domain():
                    vals = []
                    var_to_check.assign(val)
                    for var in vars:
                        vals.append(var.get_assigned_value())
                    if not c.check(vals):
                        var_to_check.prune_value(val)
                        prune_list.append((var_to_check,val))
                    var_to_check.unassign()
                if var_to_check.cur_domain_size() == 0:
